...
public class UserService {
    ...
    public User register(String email, String password, UserRole role) {
        ...
        if (role == UserRole.TA) {
            ...
            newUser.setStatus(AccountStatus.ACTIVE); // TA 注册即激活
        } else if (role == UserRole.MO) {
            newUser = new MO(email, password);
            newUser.setStatus(AccountStatus.PENDING); // MO 需管理员审核
        } else {
    ...
    /** 【新】
     * 更新账号状态（供管理员激活 MO 账号使用）
     * * @param email 目标邮箱
     * @param newStatus 新状态
     * @return 是否更新成功
     */
    public boolean updateUserStatus(String email, AccountStatus newStatus) {
        User user = userDAO.findByEmail(email);
        if (user != null) {
            user.setStatus(newStatus);
            user.setLastLogin(LocalDateTime.now());
        
        // 执行更新动作（假设 update 是 void 方法，或者是返回 boolean 的方法）
            userDAO.update(user); 
        
        // 修正点：返回 true，表示更新成功
            return true; 
        }
    // 返回 false，表示用户不存在，更新失败
        return false;
    }
