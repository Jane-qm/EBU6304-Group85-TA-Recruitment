...
public class RegisterFrame extends JFrame {
    ...
    private void initUI() {
        ...
        // 在 RegisterFrame.java 的 initUI() 按钮监听器中：
        registerButton.addActionListener(e -> {
            ...
            // 2. 调用 Service 执行注册
            UserRole role = selectedRole.equals("助教 (TA)") ? UserRole.TA : UserRole.MO;
            try {
                User result = authService.register(email, password, role);
                if (result != null) {
                    String msg = "注册成功！" + (role == UserRole.MO ? "\n注意：MO账号需等待管理员激活。" : "");
                    JOptionPane.showMessageDialog(this, msg);
                    this.dispose();
                    new LoginFrame().setVisible(true);
                } else {
                    JOptionPane.showMessageDialog(this, "注册失败：该邮箱已被占用", "错误", JOptionPane.ERROR_MESSAGE);
                }
            } catch (IllegalArgumentException ex) {
                JOptionPane.showMessageDialog(this, ex.getMessage(), "格式错误", JOptionPane.WARNING_MESSAGE);
            }
        });
