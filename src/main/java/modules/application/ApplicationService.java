...
import modules.notification.NotificationKind;
...
public class ApplicationService {
    ...
    public List<Application> listByTaUserId(Long taUserId) {
        List<Application> result = new ArrayList<>();
        for (Application application : dao.findAll()) {
    ...
    public List<Application> listByJobId(Long jobId) {
        List<Application> result = new ArrayList<>();
        for (Application application : dao.findAll()) {
    ...
    public List<Application> listByTaUserIdSorted(Long taUserId) {
        List<Application> result = listByTaUserId(taUserId);
        result.sort((a, b) -> {
    ...
    public void validateApplicationAccess(Long taUserId, Long jobId) {
        ...
        Job job = jobService.getPublishedJob(jobId);
        if (job == null) {
     ...
     * TA 拒绝 Offer
     */
    public Application rejectOffer(Long applicationId) {
        Application application = findById(applicationId);
        if (application == null) {
            throw new IllegalArgumentException("Application not found.");
        }
    ...
    public int processExpiredApplicationsForJob(Long jobId) {
        ...
        for (Application application : listByJobId(jobId)) {
     ...
     * MO 将申请标记为候选
     */
    public Application markAsWaitlisted(Long applicationId) {
        Application application = findById(applicationId);
        if (application == null) {
            throw new IllegalArgumentException("Application not found.");
        }
 ...
 * 根据职位 ID 获取候选名单中的申请
 */
public List<Application> listWaitlistedByJobId(Long jobId) {
    List<Application> result = new ArrayList<>();
    for (Application app : listByJobId(jobId)) {
 ...
 */
public List<Application> listApplicationsAwaitingReview() {
    ...
    public String buildApplicationSummary(Application application) {
        if (application == null) {
