...
public class UserService {
    ...
    public User register(String email, String password, UserRole role) {
        ...
        User newUser = switch (role) {
            ...
            default    -> throw new IllegalArgumentException("不支持的用户角色: " + role);
        };
    ...
    /**
     * 更新用户个人资料（RC-4：资料可保存与编辑）
     *
     * <p>适用于所有角色，通过 DAO 层将修改持久化到 JSON 文件。</p>
     *
     * @param user 携带最新字段值的用户对象（必须已设置 userId）
    ...
    public void updateProfile(User user) {
        ...
        if (user.getUserId() == null) {
            throw new IllegalArgumentException("用户 ID 不能为空，无法定位记录");
        }
        try {
            userDAO.update(user);
        } catch (Exception e) {
